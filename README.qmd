---
title: "Getting historical OSM infrastructure data"
format: 
   gfm
---
...
In the first method, we construct the cycling network and its categories directly from OSM tags.  
We use explicit filtering and classification rules based on combinations of:
...
These rules identify all links functioning as cycling infrastructure, which we then collapse into the four `infra_simple` classes.  
This method is fully transparent and can be adapted to different local definitions of cycling infrastructure.
...
- `R/00_setup.R`            # city tag, city name, file paths, infra_region, snapshot_version  
- `R/01_get_boundary.R`     # build city perimeter  
- `R/02_get_infrastructure.R`  # download / load OSM travel network  
- `R/03_full_network.R`     # classify full street network (highway groups)  
- `R/04_cycling_network.R`  # build and classify cycling networks (two methods)  
- `R/05_city_maps.R`         # plotting functions (full and cycling maps)

In the example, we use the **2016** OSM snapshot for **Montréal, Canada**.
...
```
To switch city (and its default year), change these values. For example, for Paris around 2023:
...
We work with historical OSM snapshots identified by version codes, for example:
...
- **Change city and default year**: update `city_name`, `city_tag`, `city_boundary_place`, `infra_region`, and `snapshot_version` in `R/00_setup.R`.
...
build_cycling_network("osmextract") # from 04
build_cycling_network("osmactive")  # from 04
```
...
#| warning: false
#| paged-print: false
# Full travel network by road type
plot_city_map_full_tm()

# Cycling infra – tag based method
plot_city_map_cycling_tag_tm()

# Cycling infra – osmactive classification
plot_city_map_cycling_osmactive_tm()
```
